...
import { fmt, mapProduct, buildAffiliateUrl } from "../../utils.js";
import { API_BASE } from "../../data/constants.js";
import { extractKeyword, buildCoupangUrl } from "./helpers.js";
...
const fetchSellerList = async (productName, productId) => {
  ...
  try {
    ...
    return (data.products || []).map(mapProduct).filter(p => p.id !== productId).slice(0, 4);
  } catch { return []; }
};

export const SellerCompareCard = ({ product }) => {
  ...
  return (
    <div className="bg-[#F7F7F8] rounded-3xl p-4">
      ...
      <div className="space-y-2">
        {sellers.map((s, i) => (
          <a key={s.id} href={buildAffiliateUrl(s.id)} target="_blank" rel="noopener noreferrer"
