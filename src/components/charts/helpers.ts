...
export const sliceHist = (hist: PricePoint[], days: number): PricePoint[] =>
  hist.slice(Math.max(0, hist.length - Math.ceil(days / 4)));

export const calcWeeklyPattern = (basePrice: number, seed: number): DayPattern[] => {
...
export const calcNextSale = (): SaleEvent | undefined => {
  ...
  const upcoming = candidates
    .map(s => ({ ...s, dday: Math.ceil((s.date.getTime() - today.getTime()) / 86400000) }))
    .filter(s => s.dday >= 0)
    .sort((a, b) => a.dday - b.dday);
  return upcoming[0];
};

export const buildTimeline = (hist: PricePoint[], _currentPrice: number): TimelineEvent[] => {
...
export const buildCoupangUrl = (keyword: string): string => {
