...
import { idToSeed, generateHistory, buildAffiliateUrl, fmt, getLocalAlerts } from "../utils.js";

const Sparkline = ({ prices, width = 60, height = 20 }) => {
...
const ShoppingCandleChart = ({ hist, width = 300, height = 80 }) => {
  ...
  const toY   = (p) => height - ((p - minP) / range) * height * 0.85 - height * 0.07;
...
export const PriceHistoryItem = ({ item, onDetail }) => {
  ...
  const hasAlert = alerts.some(a => a.productId === item.id);
  ...
  return (
    <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100">
      ...
      <div className="flex border-t border-gray-100">
        ...
        <a href={buildAffiliateUrl(item.id)} target="_blank" rel="noopener noreferrer"
...
export const BollingerSavingsChart = ({ hist }) => {
  ...
  const W       = 300, H = 80;
  const toX = (i) => (i / (bands.length - 1)) * W;
  const toY = (v) => H - ((v - minV) / range) * H * 0.85 - H * 0.07;
