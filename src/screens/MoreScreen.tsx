...
import { getLocalWishlist, fmt } from "../utils";
...
export const MoreScreen = ({ onFeedback: _onFeedback, onPrivacy, onTerms, onHowTo, user, onLogin, onLogout, showToast, onInstall, isStandalone }: MoreScreenProps) => {
    ...
    <div className="pb-10">
      ...
      <div className="px-4 py-5 space-y-4">
        ...
        <div>
          ...
          <div className="bg-[#F7F7F8] rounded-2xl overflow-hidden">
            <button onClick={() => {
              const win = window as Window & { ChannelIOBooted?: boolean; ChannelIO?: (cmd: string) => void };
              if (win.ChannelIOBooted) { win.ChannelIO?.("openChat"); }
              else if (win.ChannelIO) {
                setTimeout(() => {
                  if (win.ChannelIOBooted) win.ChannelIO?.("openChat");
